...
# Profile _score for first candidate (includes JIT compilation)
if length(cands) >= 2
    cand1 = cands[1]
    println("Profiling first candidate with context length 2 (with compilation): '$cand1'")
    t0 = time_ns()
    ...
    # Profile _score for second candidate (pure runtime)
    cand2 = cands[2]
    println("Profiling second candidate with context length 2 (pure runtime): '$cand2'")
    t2 = time_ns()
