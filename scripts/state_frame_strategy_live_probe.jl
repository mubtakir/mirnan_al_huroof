#!/usr/bin/env julia
# Live gated probe for StateFrameStrategy.
...
function _ask(gen, prompt::AbstractString; gate::Bool)
    ...
    try
        ENV["MIRNAN_ENABLE_STATE_FRAME_STRATEGY"] = gate ? "1" : "0"
...
function _print_case(gen, label::AbstractString, prompt::AbstractString)
    ...
    println("-- independent state answer --")
    mem = Gen._LEARNED_ISTINBAT_MEMORY[]
    independent = mem === nothing ? "" : Physics.state_answer(mem, String(prompt))
...
function main()
    ...
    println("StateFrameStrategy live probe")
    println("generator: small controlled")
    println("gate variable: MIRNAN_ENABLE_STATE_FRAME_STRATEGY")
    try
        Gen._LEARNED_ISTINBAT_MEMORY[] = controlled
        _print_case(gen, "controlled state", "\u0643\u064a\u0641 \u062f\u062e\u0644 \u0627\u0644\u0637\u0641\u0644\u061f")
        _print_case(gen, "yes/no guard", "\u0647\u0644 \u062f\u062e\u0644 \u0627\u0644\u0637\u0641\u0644\u061f")
        _print_case(gen, "non-state guard", "\u0645\u0627 \u0645\u0639\u0646\u0649 \u0627\u0644\u0633\u0644\u0627\u0645\u061f")
    finally
